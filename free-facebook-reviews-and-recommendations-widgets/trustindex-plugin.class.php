...
'name' => __('Free Widget Configurator', 'free-facebook-reviews-and-recommendations-widgets')
];
if ($this->is_noreg_linked()) {
$tabs[] = [
'place' => 'left',
'slug' => 'my-reviews',
'name' => __('My reviews', 'free-facebook-reviews-and-recommendations-widgets')
];
}
$tabs[] = [
'place' => 'left',
'slug' => 'get-reviews',
'name' => __('Get Reviews', 'free-facebook-reviews-and-recommendations-widgets')
];
$tabs[] = [
'place' => 'left',
'slug' => 'rate-us',
'name' => __('Rate Us', 'free-facebook-reviews-and-recommendations-widgets')
];
if (!$this->is_trustindex_connected()) {
$tabs[] = [
'place' => 'left',
'slug' => 'get-more-features',
'name' => __('Get more Features', 'free-facebook-reviews-and-recommendations-widgets')
];
}
$tabs[] = [
'place' => 'right',
...
'place' => 'right',
'slug' => 'advanced',
'name' => __('Advanced', 'free-facebook-reviews-and-recommendations-widgets')
];
return $tabs;
}
...
return '<a class="ti-btn" href="https://www.trustindex.io/?a=sys&c='. $campaignId .'" target="_blank">'. __('Create a Free Account for More Features', 'free-facebook-reviews-and-recommendations-widgets') .'</a>';
}
...
$settingsPageTitle .= mb_strtolower(__('Reviews', 'free-facebook-reviews-and-recommendations-widgets'));
}
else {
$settingsPageTitle .= strtolower(__('Reviews', 'free-facebook-reviews-and-recommendations-widgets'));
}
$topMenu = false;
...
$platformLink .= 'href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=my-reviews') . '">'. __('Review Management', 'free-facebook-reviews-and-recommendations-widgets');
}
else {
$platformLink .= 'href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php') . '">';
if (!$this->is_noreg_linked()) {
/* translators: %s: Platform name */
$platformLink .= sprintf(__('Connect %s', 'free-facebook-reviews-and-recommendations-widgets'), $this->platform_name);
}
else {
$platformLink .= __('Create Widget', 'free-facebook-reviews-and-recommendations-widgets');
}
}
$platformLink .= '</a>';
$settingsLink = '<a href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php') . '">' . __('Settings', 'free-facebook-reviews-and-recommendations-widgets') . '</a>';
...
{
if (basename($file) === basename($this->plugin_file_path)) {
$meta[] = '<a href="'. admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=get-more-features') .'">'. __('Get more Features', 'free-facebook-reviews-and-recommendations-widgets') . ' →</a>';
$meta[] = '<a href="http://wordpress.org/support/view/plugin-reviews/'. $this->get_plugin_slug() .'" target="_blank" rel="noopener noreferrer">'. __('Rate our plugin', 'free-facebook-reviews-and-recommendations-widgets') . ' <span style="color: #F6BB07; font-size: 1.2em; line-height: 1; position: relative; top: 0.05em;">★★★★★</span></a>';
}
return $meta;
}
...
sprintf(__('We have worked a lot on the free "%s" plugin.', 'free-facebook-reviews-and-recommendations-widgets'), $this->plugin_name) . '<br />' .
__('If you love our features, please write a review to help us make the plugin even better.', 'free-facebook-reviews-and-recommendations-widgets') . '<br />' .
/* translators: %s: Trustindex CEO */
sprintf(__('Thank you. Gabor, %s', 'free-facebook-reviews-and-recommendations-widgets'), 'Trustindex CEO'),
],
'not-using-no-connection' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Platform name */
'button-text' => sprintf(__('Create a free %s widget! »', 'free-facebook-reviews-and-recommendations-widgets'), $platformName),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
...
/* translators: %s: Platform name */
'text' => sprintf(__('Display %s reviews on your website.', 'free-facebook-reviews-and-recommendations-widgets'), $platformName),
],
'not-using-no-widget' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Platform name */
'button-text' => sprintf(__('Embed the %s reviews widget! »', 'free-facebook-reviews-and-recommendations-widgets'), $platformName),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
...
'redirect' => $defaultRedirect,
/* translators: %s: Platform name */
'text' => sprintf(__('Build trust and display your %s reviews on your website.', 'free-facebook-reviews-and-recommendations-widgets'), $platformName),
],
'review-download-available' => [
'type' => 'warning',
'extra-class' => "",
'button-text' => __('Download your latest reviews! »', 'free-facebook-reviews-and-recommendations-widgets'),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
'remind-later-button' => false,
...
/* translators: %s: Platform name */
'text' => sprintf(__('You can update your %s reviews.', 'free-facebook-reviews-and-recommendations-widgets'), $platformName),
],
'review-download-finished' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Service name (ChatGPT) */
'button-text' => sprintf(__('Reply with %s! »', 'free-facebook-reviews-and-recommendations-widgets'), 'ChatGPT'),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
'remind-later-button' => false,
'redirect' => '?page='. $this->get_plugin_slug() .'/settings.php&tab=my-reviews',
/* translators: %s: Platform name */
'text' => sprintf(__('Your new %s reviews have been downloaded.', 'free-facebook-reviews-and-recommendations-widgets'), $platformName),
],
];
...
.' data-ti-notice-dismiss-text="'. esc_attr(__('Dismiss', 'free-facebook-reviews-and-recommendations-widgets')) .'"'
: "";
...
$chosedPlatform = new $className($forcePlatform, $filePath, "do-not-care-14.1.1", "do-not-care-Widgets for Reviews & Recommendations", "do-not-care-Facebook");
$chosedPlatform->setNotificationParam('not-using-no-widget', 'active', false);
if (!$chosedPlatform->is_noreg_linked()) {
/* translators: %s: Platform name */
return wp_kses_post($this->frontEndErrorForAdmins(sprintf(__('You have to connect your business (%s)!', 'free-facebook-reviews-and-recommendations-widgets'), $forcePlatform)));
...
return self::get_alertbox($type, '<strong>'.$title.'</strong><br />'.$text.'<br /><i style="opacity: 0.65">('. sprintf(__('This message is not be visible to visitors in public mode.') .')</i>', 'free-facebook-reviews-and-recommendations-widgets'), false, $dismissUrl);
}
...
<p>' .
'<strong>'. __('ERROR with the following plugin:', 'free-facebook-reviews-and-recommendations-widgets') .'</strong> '. $this->plugin_name .'<br /><br />' .
__('CSS file could not saved.', 'free-facebook-reviews-and-recommendations-widgets') .' <strong>('. $this->getCssFile() .')</strong> '. __('Your widgets do not display properly!', 'free-facebook-reviews-and-recommendations-widgets') . '<br />';
...
if ($fileExists) {
$html .= __('CSS file exists and it is not writeable. Delete the file', 'free-facebook-reviews-and-recommendations-widgets');
}
else {
$html .= __('Grant write permissions to upload folder', 'free-facebook-reviews-and-recommendations-widgets');
}
$html .= '<br />' .
__('or', 'free-facebook-reviews-and-recommendations-widgets') . '<br />' .
/* translators: %s: Advanced page link */
sprintf(__("enable 'CSS internal loading' in the %s page!", 'free-facebook-reviews-and-recommendations-widgets'), '<a href="'. admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=advanced') .'>'. __('Advanced', 'free-facebook-reviews-and-recommendations-widgets') .'</a>');
}
echo wp_kses($html, [
'p' => [], 'strong' => [], 'br' => [],
...
private static $widget_recommendation_texts = array (
 ...
 'ca' => 
 array (
 'negative' => 'NOT_RECOMMEND_ICON no recomana',
 'positive' => 'RECOMMEND_ICON recomana',
 ),
 'cs' => 
 array (
 ...
 'lv' => 
 array (
 'negative' => 'NOT_RECOMMEND_ICON neiesaka',
 'positive' => 'RECOMMEND_ICON iesaka',
 ),
 'mk' => 
 array (
...
return wp_kses_post($this->frontEndErrorForAdmins(__('Free plugin features are unavailable with AMP plugin.', 'free-facebook-reviews-and-recommendations-widgets')));
}
...
$text = sprintf(__('Our exclusive "Top Rated" badge is awarded to service providers with a rating of %s and above.', 'free-facebook-reviews-and-recommendations-widgets'), self::$topRatedMinimumScore)
.'<br />'
.'<a href="'.admin_url('admin.php?page='.$this->get_plugin_slug().'/settings.php&tab=free-widget-configurator&step=2').'">'.__('Please select another widget', 'free-facebook-reviews-and-recommendations-widgets').'.</a>';
...
if (!$reviews) {
return wp_kses_post(self::get_alertbox('error', __('You do not have reviews with the current filters. <br />Change your filters if you would like to display reviews on your page!', 'free-facebook-reviews-and-recommendations-widgets')));
}
...
return $this->frontEndErrorForAdmins(__('Could not download the template for the widget.<br />Please reload the page.<br />If the problem persists, please write an email to support@trustindex.io.', 'free-facebook-reviews-and-recommendations-widgets') .'<br /><br />'. print_r($response, true));
}
...
if ($r->rating == 1) {
$ratingContent = self::$widget_recommendation_texts[$language]['negative'];
}
else {
$ratingContent = self::$widget_recommendation_texts[$language]['positive'];
}
...
'<img class="ti-recommendation-icon" src="'.$this->get_plugin_file_url('static/img/recommendation-negative.svg').'" alt="Facebook" width="15" height="15" loading="lazy" />',
'<img class="ti-recommendation-icon" src="'.$this->get_plugin_file_url('static/img/recommendation-positive.svg').'" alt="Facebook" width="15" height="15" loading="lazy" />',
...
if ($this->is_trustindex_connected()): ?>
<?php if ($ti_widgets): ?>
<h2><?php echo esc_html(__('Your saved widgets', 'free-facebook-reviews-and-recommendations-widgets')); ?></h2>
...
esc_html(__("You have no widget saved!", 'free-facebook-reviews-and-recommendations-widgets')) . " "
. "<a target='_blank' href='" . "https://admin.trustindex.io/" . "widget'>". esc_html(__("Let's go, create amazing widgets for free!", 'free-facebook-reviews-and-recommendations-widgets'))."</a>"
)); ?>
<?php endif; ?>
<?php else: ?>
<?php echo wp_kses_post(self::get_alertbox("warning",
esc_html(__("You have not set up your Trustindex account yet!", 'free-facebook-reviews-and-recommendations-widgets')) . " "
/* translators: %s: URL */
. wp_kses(sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'free-facebook-reviews-and-recommendations-widgets'), admin_url('admin.php?page='.$this->get_plugin_slug().'/settings.php&tab=advanced')))
)); ?>
<?php endif;
wp_die();
}
