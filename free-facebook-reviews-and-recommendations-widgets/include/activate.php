...
wp_die(wp_kses_post(
'<strong>' . __('Plugin activation is failed because the required database tables could not created!', 'free-facebook-reviews-and-recommendations-widgets') . '</strong><br /><br />' .
/* translators: %s: database */
sprintf(__('We got the following error from %s:', 'free-facebook-reviews-and-recommendations-widgets'), __('database', 'free-facebook-reviews-and-recommendations-widgets')) .
'<pre style="'. $preStyle .'">'. $mysqlError .'</pre>' .
'<strong>' . __('Run the following SQL codes in your database administration interface (e.g. PhpMyAdmin) to create the tables or contact your system administrator:', 'free-facebook-reviews-and-recommendations-widgets') . '</strong>' .
'<pre style="'. $preStyle .'">' . implode('</pre><pre style="'. $preStyle .'">', $sqlsToRun) . '</pre>' .
'<strong>' . __('Then try activate the plugin again.', 'free-facebook-reviews-and-recommendations-widgets') . '</strong>'
));
}
